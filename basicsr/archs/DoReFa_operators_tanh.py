...
def round_pass(x):
    ...
    # mask = (x - 0.5) * 2 < 0
    # out1 = (((x - 0.5) * 2)*((x - 0.5) * 2) + 2*((x - 0.5) * 2) + 1) / 2
    # out2 = out1 * mask.type(torch.float32) + (-((x - 0.5) * 2)*((x - 0.5) * 2) + 2*((x - 0.5) * 2) + 1) / 2 * (1-mask.type(torch.float32))
    # out3 = out2 + input.floor()
    # y=(tanh(1*(x-0.5))/tanh(0.5))/2+0.5
    x = (torch.tanh(1*(x-0.5)) / torch.tanh(torch.ones(1, device=x.device)*0.5)) / 2 + 0.5
    out3 = x + input.floor().detach()
    return x_round.detach() - out3.detach() + out3
    # input.round() - out3.detach() + out3
