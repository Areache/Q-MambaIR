...
# from .lsqquantize_V1 import Weight_Quant, Act_Quant, Act_Quant as ssm_Act_Quant, Quant_conv, Quant_linear
# from .lsqquantize_V2 import Weight_Quant, Act_Quant, Act_Quant as ssm_Act_Quant, Quant_conv, Quant_linear
from .LSQ_operator_old import QuantLinear as Quant_linear, QuantConv2d as Quant_conv, LSQ_act_quant as Act_Quant, LSQ_act_quant as ssm_Act_Quant, LSQ_weight_quant as Weight_Quant
    ...
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3, squeeze_factor=30, k_bits=32):
        ...
        if is_light_sr: # we use dilated-conv & DWConv for lightSR for a large ERF
            ...
            self.cab = nn.Sequential(
                ...
                nn.GELU(),
                Quant_conv(num_feat // compress_ratio, num_feat, 3, 1, 1, nbits_a=k_bits, nbits_w=k_bits),
                ChannelAttention(num_feat, squeeze_factor)
            )
...
class SS2D(nn.Module):
    def __init__(
        ...
        self.x_proj_weight_q = Weight_Quant(nbits_w=k_bits)
        ...
        self.dt_proj_weight_q = Weight_Quant(nbits_w=k_bits)
        ...
        # self.out_proj = out_linear(self.d_inner, self.d_model, bias=bias, nbits_w=k_bits, nbits_a=k_bits, )
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None
        # self.shortcut = nn.Parameter(torch.zeros(1,self.d_inner,1,1))
        #! Quant_SSC
        self.xs_q = ssm_Act_Quant(k_bits)
        self.dts_q = ssm_Act_Quant(k_bits)
        self.As_q = ssm_Act_Quant(k_bits)
        self.Bs_q = ssm_Act_Quant(k_bits)
        self.Cs_q = ssm_Act_Quant(k_bits) 
        self.Ds_q = ssm_Act_Quant(k_bits)
        self.dt_projs_bias_q = ssm_Act_Quant(k_bits)
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        # import pdb; pdb.set_trace()
        # mini_y = self.mamba_minimal(xs,dts,As,Bs,Cs,Ds).view(B, K, -1, L)
        ...
        # import pdb; pdb.set_trace()
        # q_x, x_scale = self.quantize_per_tensor_absmax(x.clone(), n_bits=8)
        # q_dt, dt_scale = self.quantize_per_tensor_absmax(dts.clone(), n_bits=8)
        # q_B, B_scale = self.quantize_per_tensor_absmax(B.clone(), n_bits=8)
        # q_C, C_scale = self.quantize_per_tensor_absmax(C.clone(), n_bits=8)
        # q_y, y_scale = self.quantize_per_tensor_absmax(out_y.clone(), n_bits=8) 
    ...
    def mamba_minimal(self, u, delta, A, B, C, D):
        ...
        Args:
            ...
            delta: shape (b, l, d_in) new (b, d_in, l)
            u_new: shape (b, d_in, l)    (See Glossary at top for definitions of b, l, d_in, n...)
            delta_new: shape (b, d_in, l)
            A: shape (d_in, n) new (288,10)
            B: shape (b, l, n) (1, 4, 10, 35156) Bknl
            C: shape (b, l, n) (1, 4, 10, 35156) Bknl
            D: shape (d_in,) (288)
        ...
        (b, d_in, l) = u.shape
        n = A.shape[1]
        A = A.float().view(d_in,n)
        B = B.float().reshape(b*4,l,-1)
        C = C.float().reshape(b*4,l,-1)
        u = u.float().view(b, l, d_in)
        delta = delta.float().view(b, l, d_in)
        # Discretize continuous parameters (A, B)
        # - A is discretized using zero-order hold (ZOH) discretization (see Section 2 Equation 4 in the Mamba paper [1])
        # - B is discretized using a simplified Euler discretization instead of ZOH. From a discussion with authors:
        #   "A is the more important term and the performance doesn't change much with the simplification on B"
        deltaA = torch.exp(einsum(delta, A, 'b l d_in, d_in n -> b l d_in n'))
        # deltaA = torch.exp(einsum(delta, A, 'b d_in l, d_in n -> b l d_in n'))
        # deltaA = torch.exp(einsum(dts, As, 'b d_in l, d_in n -> b l d_in n'))
        deltaB_u = einsum(delta, B, u, 'b l d_in, b l n, b l d_in -> b l d_in n')
        # deltaB_u = einsum(delta, B, u, 'b d_in l, b l n, b l d_in -> b l d_in n')
        ...
        y = y + u * D
        # delta_A = torch.exp(einsum(dts, As, 'b d_in l, d_in n -> b l d_in n'))
        # discret_A = delta_A[:, 0]
        # _, s, _ = torch.svd(discret_A[0])
        # # _, s, _ =torch.svd(torch.exp(einsum(dts, As, 'b d_in l, d_in n -> b l d_in n')))
        # eigenvalues_in = torch.max(s)
        # _, s, _ = torch.svd(As)
        # # _, s, _ =torch.svd(torch.exp(einsum(dts, As, 'b d_in l, d_in n -> b l d_in n')))
        # eigenvalues_in = torch.max(s)
        # _, s_out, _ = torch.svd(self.As_q(As))
        # eigenvalues_out = torch.max(s_out)
        # mse = torch.mean((As - self.As_q(As)) ** 2).item()
        # print(eigenvalues_in)
        # print(eigenvalues_out)
        # print(mse)
        return y
...
class QuantLSQ_MambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(QuantLSQ_MambaIR, self).__init__()
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = Quant_conv(dim, dim, 3, 1, 1, nbits_w=k_bits, nbits_a=k_bits)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(
                Quant_conv(dim, dim // 4, 3, 1, 1, nbits_w=k_bits, nbits_a=k_bits), 
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Quant_conv(dim // 4, dim // 4, 1, 1, 0, nbits_w=k_bits, nbits_a=k_bits), 
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Quant_conv(dim // 4, dim, 3, 1, 1, nbits_w=k_bits, nbits_a=k_bits))
