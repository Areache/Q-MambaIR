...
# Extended Mamba block with custom modifications and quantization support
class ExtendedMamba(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Lazy import quantized layers to avoid circular dependencies
        from basicsr.archs.arch_util_swinir import QLinears, QConv2ds
        
        self.in_proj = QLinears[qconv](self.d_model, in_proj_output_size, bias=bias, nbits_w=nbits_w, nbits_a=nbits_a)

        # conv 1d or 2d
        if conv_2d:
            self.conv = QConv2ds[qconv](
            ...
            # For 1D conv, we still use standard Conv1d as QConv2ds doesn't support 1D
            self.conv = nn.Conv1d(
                in_channels=self.d_inner,
                out_channels=self.d_inner,
                bias=conv_bias,
        ...
        self.out_proj = QLinears[qconv](self.d_inner, self.d_model, bias=bias, nbits_w=nbits_w, nbits_a=nbits_a)
        ...
        self.x_proj = [
            QLinears[qconv](self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, nbits_w=nbits_w, nbits_a=nbits_a) 
        ...
        self.dt_projs = [
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                         qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a, **factory_kwargs) for _ in range(self.scan_count)
        ]
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4,
        ...
        from basicsr.archs.arch_util_swinir import QLinears
        
        dt_proj = QLinears[qconv](dt_rank, d_inner, bias=True, nbits_w=nbits_w, nbits_a=nbits_a)
