...
# Helper function to create quantized layers with proper arguments
def _create_qconv(qconv, *args, nbits_w=4, nbits_a=4, **kwargs):
    """Create quantized Conv2d layer, handling FP32 case."""
    if qconv == 'FP32':
        return QConv2ds[qconv](*args, **kwargs)
    else:
        return QConv2ds[qconv](*args, nbits_w=nbits_w, nbits_a=nbits_a, **kwargs)

def _create_qlinear(qconv, *args, nbits_w=4, nbits_a=4, **kwargs):
    """Create quantized Linear layer, handling FP32 case."""
    if qconv == 'FP32':
        return QLinears[qconv](*args, **kwargs)
    else:
        return QLinears[qconv](*args, nbits_w=nbits_w, nbits_a=nbits_a, **kwargs)


class ShuffleAttn(nn.Module):
    def __init__(self, in_features, out_features, hidden_features=None, group=4, act_layer=nn.GELU, input_resolution=(64,64), qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        self.group = group
        ...
        self.gating = nn.Sequential(
            ...
            _create_qconv(qconv, in_features, out_features, groups=self.group, kernel_size=1, stride=1, padding=0, nbits_w=nbits_w, nbits_a=nbits_a),
            nn.Sigmoid()
        )
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., input_resolution=(64,64), qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        ...
        self.input_resolution = input_resolution
        self.fc1 = _create_qlinear(qconv, in_features, hidden_features, nbits_w=nbits_w, nbits_a=nbits_a)
        self.act = act_layer()
        self.fc2 = _create_qlinear(qconv, hidden_features, out_features, nbits_w=nbits_w, nbits_a=nbits_a)
...
class VMM(nn.Module):
    def __init__(
        ...
        self.in_proj = _create_qlinear(qconv, self.d_model, self.d_inner * 2, bias=bias, nbits_w=nbits_w, nbits_a=nbits_a)
        self.conv2d = _create_qconv(
            qconv,
        ...
        self.x_proj = (
            _create_qlinear(qconv, self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, nbits_w=nbits_w, nbits_a=nbits_a),
            _create_qlinear(qconv, self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, nbits_w=nbits_w, nbits_a=nbits_a),
            _create_qlinear(qconv, self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, nbits_w=nbits_w, nbits_a=nbits_a),
            _create_qlinear(qconv, self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, nbits_w=nbits_w, nbits_a=nbits_a),
        )
        ...
        self.dt_projs = (
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a,
                         **factory_kwargs),
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a,
                         **factory_kwargs),
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a,
        ...
        self.out_proj = _create_qlinear(qconv, self.d_inner, self.d_model, bias=bias, nbits_w=nbits_w, nbits_a=nbits_a)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None

        self.gating = ShuffleAttn(in_features=self.d_inner*4, out_features=self.d_inner*4, group=self.d_inner, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)

    @staticmethod
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4,
                qconv='FP32', nbits_w=4, nbits_a=4, **factory_kwargs):
        dt_proj = _create_qlinear(qconv, dt_rank, d_inner, bias=True, nbits_w=nbits_w, nbits_a=nbits_a)
...
class RMB(nn.Module):
    def __init__(
        ...
        self.self_attention = VMM(d_model=hidden_dim, d_state=d_state,expand=ssm_ratio,dropout=attn_drop_rate, input_resolution=input_resolution, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a, **kwargs)
        self.drop_path = DropPath(drop_path)
        self.skip_scale= nn.Parameter(torch.ones(hidden_dim))
        mlp_hidden_dim = int(hidden_dim * mlp_ratio)
        self.conv_blk = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim,input_resolution=input_resolution, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)
...
class QMaIR_ours(nn.Module):
    ...
    def __init__(self,
        ...
        super(QMaIR_ours, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.conv_first = _create_qconv(qconv, num_in_ch, embed_dim, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a)
        ...
        if resi_connection == '1conv':
            self.conv_after_body = _create_qconv(qconv, embed_dim, embed_dim, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv_after_body = nn.Sequential(
                _create_qconv(qconv, embed_dim, embed_dim // 4, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                _create_qconv(qconv, embed_dim // 4, embed_dim // 4, 1, 1, 0, nbits_w=nbits_w, nbits_a=nbits_a), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                _create_qconv(qconv, embed_dim // 4, embed_dim, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a))

        # -------------------------3. high-quality image reconstruction ------------------------ #
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            self.conv_before_upsample = nn.Sequential(
                _create_qconv(qconv, embed_dim, num_feat, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a), nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)
            self.conv_last = _create_qconv(qconv, num_feat, num_out_ch, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a)
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR (to save parameters)
            self.upsample = UpsampleOneStep(upscale, embed_dim, num_out_ch, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)

        else:
            # for image denoising
            self.conv_last = _create_qconv(qconv, embed_dim, num_out_ch, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a)
...
class RMG(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = _create_qconv(qconv, dim, dim, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(
                _create_qconv(qconv, dim, dim // 4, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                _create_qconv(qconv, dim // 4, dim // 4, 1, 1, 0, nbits_w=nbits_w, nbits_a=nbits_a), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                _create_qconv(qconv, dim // 4, dim, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a))
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, qconv='FP32', nbits_w=4, nbits_a=4):
        self.num_feat = num_feat
        m = []
        m.append(_create_qconv(qconv, num_feat, (scale**2) * num_out_ch, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a))
    ...
    def __init__(self, scale, num_feat, qconv='FP32', nbits_w=4, nbits_a=4):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                m.append(_create_qconv(qconv, num_feat, 4 * num_feat, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a))
                m.append(nn.PixelShuffle(2))
        elif scale == 3:
            m.append(_create_qconv(qconv, num_feat, 9 * num_feat, 3, 1, 1, nbits_w=nbits_w, nbits_a=nbits_a))
