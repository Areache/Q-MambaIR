...
class ssm_Act_Quant(nn.Module):
    def __init__(self, nbits=4):
        super(ssm_Act_Quant, self).__init__()
    ...
    def forward(self, x):
        ...
        # plot_tensor_histogram(x, beta=(self.beta).item(), alpha1=(self.alpha*(-7)+self.beta).item(), alpha2=(self.alpha*(8)+self.beta).item(), name= "ssm_input")
        x = x + self.beta
