...
from .BL_SSM_operator import Weight_Quant, Act_Quant, Quant_conv, Quant_linear, DDA_Quant as oneBK_N_r1
...
class SS2D(nn.Module):
    def __init__(
        ...
        #! Quant_SSC
        self.xs_q = oneBK_N_r1(k_bits)
        self.dts_q = oneBK_N_r1(k_bits)
        self.As_q = oneBK_N_r1(k_bits)
        self.Bs_q = oneBK_N_r1(k_bits)
        self.Cs_q = oneBK_N_r1(k_bits) 
        self.Ds_q = oneBK_N_r1(k_bits)
        self.dt_projs_bias_q = oneBK_N_r1(k_bits)
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        #     print("NaN found in xs!")
        # if torch.isnan(dts).any():
        #     print("NaN found in dts!")
        # if torch.isnan(As).any():
        #     print("NaN found in As!")
        # if torch.isnan(Bs).any():
        #     print("NaN found in Bs!")
        # if torch.isnan(Cs).any():
        #     print("NaN found in Cs!")
        # if torch.isnan(Ds).any():
        #     print("NaN found in Ds!")   
        # if torch.isnan(dt_projs_bias).any():
        #     print("NaN found in dt_projs_bias!")      
        ...
        if torch.isnan(out_y).any():
            print("NaN found in out_y!") 
            plot_tensor_histogram(As, name='As_nu_dda')
            plot_tensor_histogram(self.As_q(As), name='As_q_nu_dda')        
            plot_tensor_histogram(Bs, name='Bs_nu_dda')
            plot_tensor_histogram(self.Bs_q(Bs), name='Bs_q_nu_dda')
            plot_tensor_histogram(Cs, name='Cs_nu_dda')
            plot_tensor_histogram(self.Cs_q(Cs), name='Cs_q_nu_dda')
            plot_tensor_histogram(Ds, name='Ds_nu_dda')
            plot_tensor_histogram(self.Ds_q(Ds), name='Ds_q_nu_dda')
            plot_tensor_histogram(dts, name='dts_nu_dda')
            plot_tensor_histogram(self.dts_q(dts), name='dts_q_nu_dda')
            plot_tensor_histogram(xs, name='xs_nu_dda')
            plot_tensor_histogram(self.xs_q(xs), name='xs_q_nu_dda')
            plot_tensor_histogram(dt_projs_bias, name='dt_projs_bias_nu_dda')
            plot_tensor_histogram(self.dt_projs_bias_q(dt_projs_bias), name='dt_projs_bias_nu_dda')
            plot_tensor_histogram(out_y, name='q_y_nu_dda')
            import pdb; pdb.set_trace()
        ...
        #     return_last_state=False,
        # ).view(B, K, -1, L)
        # plot_tensor_histogram(out_y, name='y_nu_dda')
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if torch.isnan(y1 + y2 + y3 + y4).any():
            print("NaN found in y1234!")  
        # import pdb; pdb.set_trace()
        y = self.out_norm(y)
        if torch.isnan(y).any():
            print("NaN found in y_norm!") 
            x = torch.nan_to_num(y, nan=0.0, posinf=1.0, neginf=-1.0)
            # import pdb; pdb.set_trace()
        y = y * F.silu(z)
        if torch.isnan(y).any():
            print("NaN found in y_out!")
        if torch.isnan(F.silu(z)).any():
            print("NaN found in silu!")
        # import pdb; pdb.set_trace()
        # global index
...
class QuantBL_SSM_MambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(QuantBL_SSM_MambaIR, self).__init__()
