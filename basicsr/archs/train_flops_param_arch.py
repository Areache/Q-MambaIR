...
from .train_flops_param_operator import Quant_conv, Quant_linear, Quant_out_linear, Act_Quant, DDA_Quant, Act_Quant as Weight_Quant  
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # y_add_item(y)
        # import numpy as np
        # def calculate_boxplot_statistics(tensor):
        #     # 确保 tensor 是一个 numpy 数组
        #     data = tensor.cpu().numpy().flatten()  # 将 tensor 转为 numpy 数组，并展平
            
        #     # 计算第 25 百分位数 (Q1), 第 75 百分位数 (Q3)
        ...
        #     # 计算去除异常值的最小值和最大值
        #     min_value = Q1 - 1.5 * IQR  # 最小值（去除异常值的下限）
        #     max_value = Q3 + 1.5 * IQR  # 最大值（去除异常值的上限）

        #     # 返回计算结果
        #     return Q1, Q3, IQR, median, min_value, max_value
        # Q1, Q3, IQR, median, min_value, max_value = calculate_boxplot_statistics(y)
        # print(f"Q1: {Q1}")
        # print(f"Q3: {Q3}")
        # print(f"IQR: {IQR}")
        # print(f"Median: {median}")
        # print(f"Min Value (lower bound): {min_value}")
        # print(f"Max Value (upper bound): {max_value}")
        # import pdb; pdb.set_trace()
        out = self.out_proj(y)
        # alpha_add_item(self.out_proj.linear_act_quant.alpha)
        # alpha_add_item(7.5*(self.out_proj.linear_act_quant.weight1*(y.mean()).abs()+self.out_proj.linear_act_quant.weight2 \
        #     *y.std()+self.out_proj.linear_act_quant.weight3*y.min()+ \
        #     self.out_proj.linear_act_quant.weight4*y.max()).abs())
        # beta_add_item(self.out_proj.linear_act_quant.beta)
...
class our_flops_param_QMambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(our_flops_param_QMambaIR, self).__init__()
    ...
    def forward(self, x):
        ...
        # from analysis.box_plt import box_plot
        # inverted_list = [-x for x in box_alpha]
        # box_plot(box_y,inverted_list, box_alpha)
        # import pdb;pdb.set_trace()
...
# def y_add_item(item):
#     global global_list  # 声明使用全局变量
#     box_y.append(item)

# def alpha_add_item(item):
#     global global_list  # 声明使用全局变量
#     box_alpha.append(item)

# def beta_add_item(item):
#     global global_list  # 声明使用全局变量
#     box_beta.append(item)
