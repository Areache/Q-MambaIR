...
# from .quantir_operator import MatMul_Weight_Quant, MatMul_Act_Quant, Quant_conv, Quant_linear
from .quantir_operator import Weight_Quant, Act_Quant, Quant_conv, Quant_linear, Quant_out_linear
# from .quantir_operator import Quant_conv, Quant_linear
...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3, squeeze_factor=30, k_bits=32):
        ...
        if is_light_sr: # we use dilated-conv & DWConv for lightSR for a large ERF
            ...
            # self.learnable_shortcut1 = nn.Parameter(torch.zeros(1))
            # self.learnable_shortcut2 = nn.Parameter(torch.zeros(1))
            # self.learnable_shortcut3 = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x):
        ...
        # x = self.cab[0](x)
        # x = self.cab[1](x) + x*self.learnable_shortcut1
        # x = self.cab[2](x)
        # x = self.cab[3](x)
        # x = self.cab[4](x) + x*self.learnable_shortcut2
        # x = self.cab[5](x) + x*self.learnable_shortcut3
...
class SS2D(nn.Module):
    def __init__(
        ...
        del self.x_proj
        self.x_proj_act_q = Act_Quant(nbits=k_bits)
        ...
        self.dt_proj_act_q = Act_Quant(nbits=k_bits)
        ...
        # self.learnable_inproj_1 = nn.Parameter(torch.zeros(1))
        # self.learnable_inproj_2 = nn.Parameter(torch.zeros(1))
        # self.dwconv_shortcut = nn.Parameter(torch.ones(1, self.d_inner, 1, 1))
        # self.ssm_shortcut = nn.Parameter(torch.zeros(1, 1, 1, self.d_inner))
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        xz = self.in_proj(x)
        # plot_tensor_histogram(xz, name='in_proj_out')
        x, z = xz.chunk(2, dim=-1)
        z = F.silu(z)
        # #todo shortcut_inproj
        # x = shortcut_inproj*self.learnable_inproj_1 + x
        # z = shortcut_inproj*self.learnable_inproj_2 + z
        ...
        #todo shortcut_dwconv
        # x = shortcut_dwconv + self.dwconv_shortcut*x
        # import pdb;pdb.set_trace()
        # plot_tensor_histogram(x, name='conv2d_out_with_shortcut')

        # shortcut_ssm = x.permute(0, 2, 3, 1).contiguous()
        ...
        y1 = self.out_norm(torch.transpose(y1, dim0=1, dim1=2).contiguous().view(B, H, W, -1)) * z
        y2 = self.out_norm(torch.transpose(y2, dim0=1, dim1=2).contiguous().view(B, H, W, -1)) * z
        y3 = self.out_norm(torch.transpose(y3, dim0=1, dim1=2).contiguous().view(B, H, W, -1)) * z
        y4 = self.out_norm(torch.transpose(y4, dim0=1, dim1=2).contiguous().view(B, H, W, -1)) * z
        # plot_tensor_histogram(y1, name='y1_after_norm')
        # plot_tensor_histogram(y2, name='y2_after_norm')
        # plot_tensor_histogram(y3, name='y3_after_norm')
        # plot_tensor_histogram(y4, name='y4_after_norm')
        # y = y1 + y2 + y3 + y4
        # y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        
        #todo shortcut_ssm
        # y = shortcut_ssm * self.ssm_shortcut + y
        # plot_tensor_histogram(y, name='out_norm_in')
        # y = self.out_norm(y)
        # plot_tensor_histogram(y, name='out_norm_out')
        # import pdb;pdb.set_trace()
        # y = y * F.silu(z)
        # import pdb; pdb.set_trace()
        # import pdb;pdb.set_trace()
        # plot_tensor_histogram(y, name='out_proj_in')
        # plot_tensor_histogram(y, name='out_proj_in_with_shortcut')
        out = self.out_proj(y1, y2, y3, y4)
        # plot_tensor_histogram(out, name='out_proj_out')
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(depth):
            ...
            # self.learnable_shortcut += nn.Parameter(torch.zeros(1), requires_grad=True)
    ...
    def forward(self, x, x_size):
        ...
        for i in range(self.depth):
            ...
            if self.use_checkpoint:
                ...
                x = self.blocks[i](x, x_size) # + self.learnable_shortcut[i].to(x) * x
...
class QuantIR_MambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(QuantIR_MambaIR, self).__init__()
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size):
        return self.patch_embed(self.conv(self.patch_unembed(
            self.residual_group(x, x_size), x_size))) + x # *self.learnable_shortcut
