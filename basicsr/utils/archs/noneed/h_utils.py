...
def linear_quantize(input, scale, zero_point, inplace=False):
    ...
    # inplace operation without creating new tensor
    if inplace:
...
class SymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, specified_scale=None):
        ...
        # avoid numerical overflow This function limits (or clamps) Any values in the tensor outside 
        # this range [-2 ** (k - 1), 2 ** (k - 1) - 1] k=8: [-128, 127] are set to the nearest boundary.
