...
from .APOT_operator import  MUL_Weight_Quant, Act_Quant, Quant_conv, ss2d_dwconv, Quant_linear, in_out_linear
# from .lsq_zz_operator import N2UQ_linear as Quant_linear, N2UQ_conv as Quant_conv #, Act_N2UQ as Act_Quant
# from .QuantSR_operator import QuantSR_act as Quant_act 
...
class CAB(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.cab[0](x)
        # x = self.cab[1](x) + x*self.skip_scale1
        # x = self.cab[2](x)
        # x = self.cab[3](x)
        # x = self.cab[4](x) + x*self.skip_scale2
        # x = self.cab[5](x) + x*self.skip_scale3
        return self.cab(x)
        # return x
        # return self.cab(x) 
...
class SS2D(nn.Module):
    def __init__(
        ...
        self.x_proj_weight_q = MUL_Weight_Quant(nbits=k_bits, in_features=self.d_inner)
        ...
        self.dt_proj_weight_q = MUL_Weight_Quant(nbits=k_bits, in_features=self.dt_rank)
        ...
        # self.pool = nn.AdaptiveAvgPool1d(output_size=self.d_model)
        # self.shortcut = nn.Parameter(torch.zeros(1,self.d_inner,1,1))
        # self.shortcut_y = nn.Parameter(torch.zeros(1,1,1,self.d_model))
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        #TODO shortcut between DWCNN
        # x = self.act(self.conv2d(x) + x*self.shortcut)
        ...
        y = y * F.silu(z)
        # _, _, _, C_y = y.shape
        # y_res = y.reshape(B,H*W,C_y).contiguous()
        # import pdb; pdb.set_trace()
        # y_res = F.max_pool2d(y.permute(0, 1, 3, 2) , kernel_size=(13, 1), stride=(1, 1)).permute(0, 1, 3, 2)
        # y_res = F.interpolate(y_res, size=60, mode='linear', align_corners=False).reshape(B,H,W,C)
        # y_res = self.pool(y).reshape(B,H,W,C)
        # out = self.out_proj(y) + y_res * self.shortcut_y
        out = self.out_proj(y)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        #TODO shortcut1 for SS2D
        # self.skip_scaleSS = nn.Parameter(torch.zeros(hidden_dim))
...
class QuantApot_MambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(QuantApot_MambaIR, self).__init__()
        # global is_plot
        # # global is_had 
        # is_plot = False
        is_had = False
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = Quant_conv(dim, dim, 3, 1, 1, k_bits = k_bits)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(
                Quant_conv(dim, dim // 4, 3, 1, 1, k_bits = k_bits), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Quant_conv(dim // 4, dim // 4, 1, 1, 0, k_bits = k_bits), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Quant_conv(dim // 4, dim, 3, 1, 1, k_bits = k_bits))
