...
class LTQ_yj(nn.Module):
    ...
    def __init__(self,in_features, nbits=4):
        ...
        # self.start = nn.Parameter(torch.Tensor([0]), requires_grad=True)
        # self.a = nn.Parameter(torch.Tensor([self.interval]* self.n_val), requires_grad=True)
        self.a = nn.Parameter(torch.Tensor([init_range / self.n_val]* (2*self.n_val+1)), requires_grad=True)
        # self.b = nn.Parameter(torch.Tensor([init_range / self.n_val]* int((self.n_val-1)/2)), requires_grad=True)
        self.register_buffer('init_state', torch.zeros(1))
        # self.register_buffer('self.interval', torch.zeros(1))
    ...
    def build_power_value(self, B=4, additive=True):
        base_a = [0.]
        base_b = [0.]
        base_c = [0.]
        if additive:
        ...
        tyler = values.tolist()
        negative_list = [-x for x in reversed(tyler[1:])]
        additive_pot = tyler + negative_list
    ...
    def round_ltq(self, x):
        a_pos = self.a
        additive_pot = self.build_power_value(B=4, additive=True)
        # import pdb; pdb.set_trace 
    ...
    def forward(self, x):
        ...
        # print(self.a)
        # import pdb; pdb.set_trace()
        tyler = self.build_power_value(B=4, additive=True)
        if self.training and self.init_state == 0:
            if x.min() < -1e-5:
                self.signed.data.fill_(1)
            if self.signed == 1:
