...
from .lsq_zz_operator import Weight_Quant, Act_Quant, Quant_conv, Quant_linear, in_out_linear
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # index += 1
        #TODO shortcut1 between out_proj
        # _, _, _, C_y = y.shape
        ...
        # y_res = self.pool(y_res).reshape(B,H,W,C)
        out = self.out_proj(y) + y_res * self.shortcut_y
        # out = self.out_proj(y)
        loss = self.shortcut_y.abs().max()
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        #TODO shortcut4 for SS2D
        self.skip_scaleSS = nn.Parameter(torch.zeros(hidden_dim))
        #TODO shortcut4 for CAB
        self.skip_scaleCAB = nn.Parameter(torch.zeros(hidden_dim))
    ...
    def forward(self, input, x_size):
        ...
        x = self.ln_1(input)
        # x, loss = self.self_attention(x)
        # x = input*self.skip_scale + self.drop_path(x)
        #TODO shortcut4 for SS2D
        atte_x, loss = self.self_attention(x)
        x = input*self.skip_scale + self.drop_path(atte_x + x*self.skip_scaleSS)
        # x = x*self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        x = x*self.skip_scale2 + self.ln_2(x)*self.skip_scaleCAB + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
    ...
    def forward(self, x, x_size):
        ...
        for i in range(self.depth):
            ...
            if self.use_checkpoint:
                ...
                x, loss_blcok = self.blocks[i](x, x_size)
                loss += loss_blcok
        # import pdb;pdb.set_trace()
...
class QuantSS_MambaIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 k_bits=32,
                 **kwargs):
        super(QuantSS_MambaIR, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for lightweight SR
            x = self.conv_first(x)
            out, loss = self.forward_features(x)
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        out, loss = self.residual_group(x, x_size)
        
        return self.patch_embed(self.conv(self.patch_unembed(out, x_size))) + x, loss
