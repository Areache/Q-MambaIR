...
class FlatQuantizedLinear(nn.Module):
    def __init__(self,  linear: nn.Linear, w_bits=4, a_bits=4, w_asym=True, a_asym=False, lac=True
                 , lwc=True, a_groupsize=-1):
        ...
        self.weight_quantizer = WeightQuantizer()
        self.weight_quantizer.configure(w_bits, perchannel=True, sym=not(w_asym), mse=False)
        self.act_quantizer = ActivationQuantizer(bits=a_bits, sym=not(a_asym), lac=lac, groupsize=a_groupsize, )
    ...
    def reparameterize(self, qa_trans=None, out_trans=None):
        ...
        self.linear.weight = weight.to(ori_dtype)
