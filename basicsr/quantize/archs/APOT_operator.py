...
class LTQ_yj(nn.Module):
    ...
    def round_ltq(self, x):
        a_pos = self.a
        additive_pot = self.build_power_value(B=4)
        ...
        for i in range(len(a_pos)):
            # import pdb; pdb.set_trace() 
            k_p = 20 - (a_pos[i]-a_pos[i-1])

            if i == 0:
                thre_forward_p = self.start
                thre_backward_p = self.start
                ...
                x_backward = torch.where(x > thre_backward_p, y_step_0,  0.1*x+additive_pot[i]-0.1*a_pos[i])
    ...
    def forward(self, x):
        ...
        if self.training and self.init_state == 0:
            ...
            tyler = self.build_power_value(B = 4)
            self.a.data = torch.Tensor(tyler).to(x.device)
            # self.b.data = torch.Tensor(tyler).to(x.device)
            self.init_state.fill_(1)
