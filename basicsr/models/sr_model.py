...
class SRModel(BaseModel):
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            ...
            # self.net_g = torch.quantization.convert(self.net_g.to('cpu'), inplace=True)
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq)
            self.net_g.train()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        metric_data = dict()
        if use_pbar:
            pbar = tqdm(total=len(dataloader), unit='image', colour="green",)
...
@MODEL_REGISTRY.register()
class QATSRModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(QATSRModel, self).__init__(opt)
        ...
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_g', 'params')
            self.load_network(self.net_g, load_path, self.opt['path'].get('strict_load_g', True), param_key)
            self.net_g = QAT(model_fp32=self.net_g)
            quantization_config = torch.quantization.get_default_qconfig("fbgemm")
            self.net_g.qconfig = quantization_config
            print(self.net_g.qconfig)
            torch.quantization.prepare_qat(self.net_g, inplace=True)
            print(' # # # # # # # # # # # # # # #  # # # # # # ')
    ...
    def save(self, epoch, current_iter):
        if hasattr(self, 'net_g_ema'):
            ...
            self.save_network(torch.quantization.convert(self.net_g.to('cpu'), inplace=True), 'net_g', current_iter)
        self.save_training_state(epoch, current_iter)
